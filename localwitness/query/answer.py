...
SYSTEM_PROMPT = (
    "You are LocalWitness, a private, offline second brain. Answer ONLY using "
    "the provided context. Every claim must carry a citation in [brackets], "
    ...
    "for each, e.g. [interview_note.m4a @ 00:00][vendor_contract.pdf, page 2]."
    "\n\n"
    "GROUNDING RULE — this outranks every other instruction.\n"
    "Answer if, and ONLY if, the context genuinely contains the information "
    "asked for. There are two ways to fail, and both are equally bad:\n"
    "1. GUESSING. Retrieved text is selected by topic similarity, so it will "
    "often discuss a subject NEAR the question without ever answering it. A "
    "passage on a related topic is NOT an answer. Never infer, deduce, assume, "
    "combine, or extrapolate an answer out of related facts. If the specific "
    "thing asked for is simply not stated, reply with exactly \"That's not in "
    "my notes.\" and nothing else — no citation, no partial answer.\n"
    "2. REFUSING SOMETHING THE NOTES DO ANSWER. If the context states the fact, "
    "ANSWER it — even when it is worded differently from the question. Different "
    "wording is fine; the fact being present is what matters. Refusing a "
    "question your notes genuinely answer is just as much a failure as "
    "guessing.\n"
...
def build_prompt(question: str, hits: list[dict]) -> str:
    ...
    return (
        f"Context:\n{context}\n\nQuestion: {question}\n\n"
        "First decide: does the context above actually contain the information "
        "this question asks for? It may be worded differently than the question "
        "— that is fine, answer it. But if the context only touches on a "
        "related subject and you would have to infer or piece the answer "
        f'together, reply with exactly "{FALLBACK}" and nothing else.\n\n'
        "If the answer is there, answer using only the context above. End every sentence "
        f"with the citation of the chunk it came from, like [{example}]; if a "
